...
void ImGuiSetting::CreateImGui(const HWND Hwnd_, ID3D12Device* pDevice_, ID3D12DescriptorHeap* pDescriptorHeap_)
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    ImGui_ImplDX12_Init(pDevice_, 3,
        ...
        pDescriptorHeap_->GetCPUDescriptorHandleForHeapStart(),
        pDescriptorHeap_->GetGPUDescriptorHandleForHeapStart()
    );
    ImGui::StyleColorsClassic();
}

void ImGuiSetting::IOSettings(HWND Hwnd_)
{
    // ImGuiドッキングのフラグ
...
void ImGuiSetting::Render()
{
    ImGui::Render();
}

void ImGuiSetting::RenderDrawData(ID3D12GraphicsCommandList* pCmdList_)
{
    ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), pCmdList_);
}

void ImGuiSetting::Cleanup()
{
    // Cleanup
