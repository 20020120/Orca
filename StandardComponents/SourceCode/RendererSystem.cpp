...
{
    ORCA_MAKE_UNIQUE(mpRenderPipeLineHolder);
}
...
{
    mpRenderPipeLineHolder->Set(OrcaGraphics::Shader::ShaderType::Sample, pCmdList_.Get());
}
...
void System::RenderSystem::Render(const Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& pCmdList_) const
{
    ...
    {
        mpRenderPipeLineHolder->Set(p->mShaderType, pCmdList_.Get());
